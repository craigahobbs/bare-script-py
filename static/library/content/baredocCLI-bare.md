The "baredocCLI.bare" include library provides the main entry point for the baredoc command-line
interface (CLI), baredocCLI. baredocCLI is used to generate a library JSON resource for consumption
by the [baredoc application](#var.vGroup='baredoc.bare'&_top).

To create a library JSON resource, include and execute the `baredocCLIMain` function from the
command line:
...
By default, the library JSON is logged (to stdout). You can specify an output file by using the
output argument, "vOutput":
...
baredocCLI documentation comments begin with the "$" character, followed immediately by a keyword
