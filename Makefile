...
TILDE_INDEX = INDEXES[len(PHRASES)]
PHRASE_MAP = {INDEXES[ix_phrase]: phrase for ix_phrase, phrase in enumerate(PHRASES)}
PHRASE_MAP[TILDE_INDEX] = '~'

def decode(text):
    parts = text.split('~')
    result = [parts[0]]
    for part in parts[1:]:
        result.append(PHRASE_MAP[part[0]])
...
"""
''']
phrases_text = ',\n'.join(f'    {json.dumps(char)}: {json.dumps(phrase)}' for char, phrase in PHRASE_MAP.items())
parts.append(f'''
...
for include_path in include_paths:
    ...
    constant_name = 'INCLUDE_SOURCE_' + re.sub(r'([A-Z]+)', r'_\1', include_name).upper()
    ...
    model_encoded = model_text.replace('~', '~' + TILDE_INDEX)
    for ix_phrase, phrase in enumerate(PHRASES):
        model_encoded = model_encoded.replace(phrase, '~' + INDEXES[ix_phrase])
    if decode(model_encoded) != model_text:
        raise ValueError(f'{include_name}.bare compressed model round-trip failure')
    model_lines_text = ',\n'.join(
        "    '" + chunk.replace('\\', '\\\\').replace("'", "\\'") + "'" for chunk in re.findall('.{1,120}', model_encoded)
    )
    parts.append(f"\n\n# {include_name}.bare\n{constant_name} = _include_source_decode(''.join([\n{model_lines_text}\n]))\n")
    system_includes.append(f"    '{include_name}.bare': {constant_name}")
